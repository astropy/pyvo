...
* `TOPCAT <https://www.star.bristol.ac.uk/mbt/topcat>`_, which is a tool to explore tabular data.
...
start up the `pyvo.samp` hub. You can start this using the following
command::
...
`pyvo.samp.SAMPIntegratedClient` class. Once TOPCAT is open, first instantiate a
`pyvo.samp.SAMPIntegratedClient` instance and then connect to the hub::
...
:meth:`~pyvo.samp.integrated_client.SAMPIntegratedClient.notify_all`::
...
:meth:`~pyvo.samp.integrated_client.SAMPIntegratedClient.get_registered_clients` method to
...
:meth:`~pyvo.samp.integrated_client.SAMPIntegratedClient.notify` method::
...
messages from the hub. As before, we instantiate a `pyvo.samp.SAMPIntegratedClient`
...
We can now use the
:meth:`~pyvo.samp.integrated_client.SAMPIntegratedClient.bind_receive_call`
and
:meth:`~pyvo.samp.integrated_client.SAMPIntegratedClient.bind_receive_notification`
...
make use of the `pyvo.samp.SAMPIntegratedClient` class. Once Aladin or DS9 are open,
first instantiate a `pyvo.samp.SAMPIntegratedClient` instance and then connect to the hub
as before::
