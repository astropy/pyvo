...
lazy dogs."""
        ...
        self.assertEquals(reg.para_format_desc(inp, 38), out)

    def test_para_detect(self):
        for t in self.splittests:
            self.assertEquals(reg.para_format_desc(t), "one\n\ntwo")
...
work on n^2 paragraphs."""
        ...
        text = reg.para_format_desc(inp, 40)
        if text != out:
